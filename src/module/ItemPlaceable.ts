import { STAIRWAY_DEFAULTS } from './ItemPlaceableConfig.js';
import { ItemPlaceableControlIcon } from './ItemPlaceableControlIcon.js';

/**
 * An ItemPlaceable is an implementation of PlaceableObject which represents a teleport between to points.
...
export class ItemPlaceable extends PlaceableObject {
  ...
  static resetConnectionTarget() {
    ItemPlaceable.connectionTarget = null;
  }
  ...
  get targetScene() {
    if (this.onScene) {
      ...
      // find target scene
      return game.scenes.find((scene) => scene.id === this.data.scene);
    }
  }
  ...
  get isConnectionTarget() {
    if (ItemPlaceable.connectionTarget) {
      ...
      return scene === canvas.scene.id && name === this.data.name;
    }
  ...
  async draw() {
    ...
    this.controlIcon = this.addChild(
      new ItemPlaceableControlIcon({ label: this.label, textStyle: this.labelTextStyle, texture: this.icon }),
    );
  ...
  updateOtherPlaceable() {
    ...
    if (this.otherPlaceable === false) {
      ...
      const others = canvas.itemPlaceables.placeables.filter(
  ...
  _onCreate(...args) {
    ...
    canvas.controls.createItemPlaceableControl(this);

    const { targetScene, targetData } = this.target;
    if (targetData) {
      // sync partner animate option
      const data = { animate: this.data.animate };
      ...
      (targetScene || canvas.scene).updateEmbeddedDocuments('ItemPlaceable', [{ _id: targetData._id, ...data }]);
    }
